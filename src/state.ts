...
export async function ensureState(paths: StatePaths): Promise<DeepcleanConfig> {
  await Promise.all([
    ...
    mkdir(paths.opportunitiesDir, { recursive: true }),
    mkdir(paths.campaignsDir, { recursive: true }),
    mkdir(paths.qualityProfilesDir, { recursive: true }),
    mkdir(paths.qualityResultsDir, { recursive: true }),
    mkdir(paths.analyzerSetupDir, { recursive: true }),
