...
## Purpose
Define how Deepclean collects local, structured maintainability evidence before model synthesis.
## Requirements
...
### Requirement: Optional external analyzer ingestion
The system SHALL ingest source-safe outputs from established analyzers when configured or present, while continuing to run without those tools.

#### Scenario: SARIF output is present
- **WHEN** a configured SARIF file exists in the target repository
- **THEN** Deepclean normalizes relevant results into evidence records with tool provenance and file locations

#### Scenario: jscpd command is enabled but unavailable
- **WHEN** `externalAnalyzers.jscpd.enabled` is true and the configured command cannot run
- **THEN** Deepclean records a structured diagnostic and continues with remaining adapters
